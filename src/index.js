...
function calculateSafeMPECDHAddress(safeAddress) {
  return ethers.getCreate2Address(CREATE_CALL_LIB,
    ethers.keccak256(safeAddress), ethers.keccak256(bytecode) )
}
...
async function proposeDeploySafeMPECDH(signer, safeAddress) {
  ...
  return { safeTxHash, safeAddress, mpecdhAddress: calculateSafeMPECDHAddress(safeAddress) }
}
